...
<% 
	...
	///////////////////////////////////////////////////////////// 내 책장
if(mem!=null && mem.getMemberId().equals(libraryOwner)){
%>
    ...
    <% ArrayList<Book> listLB = (ArrayList<Book>)request.getAttribute("listLB"); %>        
         ...
         <% for(Book likeB : listLB) { %>
                ...
                <input type="hidden" value="<%=likeB.getBookId()%>"/>
                    <div class="checkInPopUp">
                        <span class="minusBookInPopUp minusBookInPopUp<%=likeB.getBookId()%>">
                        ...
                        <span class="plusBookInPopUp plusBookInPopUp<%=likeB.getBookId()%>">
      ...
      <script>
		...
		function addBookInCase(data){
			...
			$.ajax({
				url : '/oneBookCase.rw',
				data : {'bookCaseId':data},
				type : 'post',
				success : function(data){
					if(data.bookIdList!=null){
						for(var i=0; i<data.bookIdList.length; i++){
							<% for(Book likeB : listLB) { %>
								if(data.bookIdList[i]=='<%=likeB.getBookId()%>'){
									$('.minusBookInPopUp'+'<%=likeB.getBookId()%>').children().css('display','none');
									$('.plusBookInPopUp'+'<%=likeB.getBookId()%>').children().css('display','inline');
								}
							<% } %>
						}
					}
