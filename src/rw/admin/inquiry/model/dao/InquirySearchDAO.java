...
public class InquirySearchDAO {
	...
	public ArrayList<Inquiry> searchInquiry(Connection conn, int currentPage, int recordCountPerPage, Date dateFrom,
			Date dateTill) {
		...
		String query ="select * " + "from (select row_number() over (order by inquiry_no desc)"
				+ "as row_num, inquiry.* from inquiry where ( inquiry_date between ? and ? ))"
	...
	public String getPageNavi(Connection conn, int currentPage, int recordCountPerPage, int naviCountPerPage,
		...
		if (startNavi != 1) {
			...
			sb.append("<li class='page-item'><a class='page-link' href='/searchInquiry.ad?currentPage=" + (startNavi - 1)
		...
		for (int i = startNavi; i <= endNavi; i++) {
			if (i == currentPage) {
				sb.append("<li class='page-item'><a class='page-link' href='/searchInquiry.ad?currentPage=" + i
						+ "&dateFrom=" + dateFrom.toString() + "&dateTill=" + dateTill.toString()
		...
		if (endNavi != pageTotalCount) {
			sb.append("<li class='page-item'><a class='page-link' href='/searchInquiry.ad?currentPage=" + (endNavi + 1)
					+ "&dateFrom=" + dateFrom.toString() + "&dateTill=" + dateTill.toString()
	...
	public ArrayList<Inquiry> searchInquiry(Connection conn, int currentPage, int recordCountPerPage, Date dateFrom,
		...
		String query ="select * " + "from (select row_number() over (order by inquiry_no desc)"
				+ "as row_num, inquiry.* from inquiry where ( inquiry_date between ? and ? ) and ( "+category+" like ? ))"
				+ "where row_num between ? and ?";
	...
	public int totalCount(Connection conn, Date dateFrom, Date dateTill, String keyword, String category) {
		...
		int totalCount = 0;
		String query = "select count(*) as totalCount from inquiry "
				+ "where (inquiry_date between ? and ?) and ("+category+" like ? )";
